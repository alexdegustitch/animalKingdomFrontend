...
function AnimalInfo() {
  ...
  useEffect(() => {
    const getAnimal = async () => {
      const response1 = await axios.get(`https://localhost:9000/animals/findAnimal/${id}`);
      ...
      const config = {
        method: "post",
        url: "https://localhost:9000/animals/animalsSameOrder",
        headers: {
  ...
  useEffect(() => {
    axios
      .get(`https://localhost:9000/animals/findAnimal/${id}`)
      .then((response) => {
